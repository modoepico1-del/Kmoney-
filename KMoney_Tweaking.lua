...
LogoHolder.Size = UDim2.new(0,62,0,76); LogoHolder.Position = UDim2.new(0,12,0.3,-38)
LogoHolder.BackgroundTransparency = 1; LogoHolder.BorderSizePixel = 0
LogoHolder.Visible = false; LogoHolder.ZIndex = 10; LogoHolder.Active = true; LogoHolder.Parent = ScreenGui
local LogoBtn = Instance.new("TextButton")
LogoBtn.Size = UDim2.new(0,50,0,50); LogoBtn.Position = UDim2.new(0.5,-25,0,0)
LogoBtn.BackgroundColor3 = Color3.fromRGB(0,0,0); LogoBtn.BorderSizePixel = 0
LogoBtn.Text = "$K"; LogoBtn.TextColor3 = VIO; LogoBtn.Font = Enum.Font.GothamBold
LogoBtn.TextSize = 18; LogoBtn.ZIndex = 10; LogoBtn.Parent = LogoHolder
Instance.new("UICorner",LogoBtn).CornerRadius = UDim.new(1,0)
local LogoStroke = Instance.new("UIStroke"); LogoStroke.Color = VIO; LogoStroke.Thickness = 2; LogoStroke.Parent = LogoBtn
local logoDragging,logoDragStart,logoStartPos = false,nil,nil
LogoBtn.InputBegan:Connect(function(inp)
    if inp.UserInputType==Enum.UserInputType.MouseButton1 then
        logoDragging=true; logoDragStart=inp.Position; logoStartPos=LogoHolder.Position end
end)
UserInputSvc.InputChanged:Connect(function(inp)
    if logoDragging and inp.UserInputType==Enum.UserInputType.MouseMovement then
        ...
        LogoHolder.Position=UDim2.new(logoStartPos.X.Scale,logoStartPos.X.Offset+d.X,logoStartPos.Y.Scale,logoStartPos.Y.Offset+d.Y) end
end)
UserInputSvc.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then logoDragging=false end end)
task.spawn(function()
    while true do
...
local kmoneyChars={"K","M","O","N","E","Y"}; local letterLabels={}
local lspc=8; local lstart=math.floor((62-#kmoneyChars*lspc)/2)
for i,ch in ipairs(kmoneyChars) do
    local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(0,lspc,0,11); lbl.Position=UDim2.new(0,lstart+(i-1)*lspc,0,56)
    lbl.BackgroundTransparency=1; lbl.Text=ch; lbl.TextColor3=Color3.fromRGB(20,20,20)
    lbl.Font=Enum.Font.GothamBold; lbl.TextSize=8; lbl.TextXAlignment=Enum.TextXAlignment.Center
    lbl.ZIndex=10; lbl.Parent=LogoHolder; letterLabels[i]=lbl
end
local neonT=0
RunService.Heartbeat:Connect(function(dt)
    ...
    for i,lbl in ipairs(letterLabels) do
        local b=math.floor(math.sin(neonT+i*0.8)*12+16); lbl.TextColor3=Color3.fromRGB(b,b,b) end
end)

-- MAIN FRAME
local HW=220
local MainFrame=Instance.new("Frame")
MainFrame.Name="MainFrame"; MainFrame.Size=UDim2.new(0,HW,0,400)
MainFrame.Position=UDim2.new(0.5,-(HW/2),0.5,-200)
MainFrame.BackgroundColor3=Color3.fromRGB(10,10,12); MainFrame.BackgroundTransparency=0.45
MainFrame.BorderSizePixel=0; MainFrame.Active=true; MainFrame.Draggable=true; MainFrame.ClipsDescendants=true
MainFrame.Parent=ScreenGui
Instance.new("UICorner",MainFrame).CornerRadius=UDim.new(0,12)
local MainStroke=Instance.new("UIStroke"); MainStroke.Color=VIO; MainStroke.Thickness=2; MainStroke.Parent=MainFrame
task.spawn(function()
    while true do
...
RunService.RenderStepped:Connect(function(dt)
    ...
    local nr=math.floor(170+pulse*50); local nb=math.floor(210+pulse*45)
...
-- TITLE BAR
local TitleBar=Instance.new("Frame"); TitleBar.Size=UDim2.new(1,0,0,70)
...
local CloseBtn=Instance.new("TextButton"); CloseBtn.Size=UDim2.new(0,20,0,20); CloseBtn.Position=UDim2.new(1,-26,0,4)
...
-- ============================================
-- TOGGLE HELPER — sin switch visual
-- Click activa (fondo violeta oscuro + borde brillante)
-- Click de nuevo desactiva (vuelve a oscuro)
-- ============================================
...
local function MakeToggle(name,yPos,callback,height,saveKey)
    ...
    Btn.Size=UDim2.new(0,BW,0,height); Btn.Position=UDim2.new(0.5,-(BW/2),0,yPos)
...
-- FOV SLIDER
local FOVBox=Instance.new("Frame"); FOVBox.Size=UDim2.new(0,BW,0,42); FOVBox.Position=UDim2.new(0.5,-(BW/2),0,Y)
FOVBox.BackgroundTransparency=0.55; FOVBox.BackgroundColor3=BLACK; FOVBox.BorderSizePixel=0; FOVBox.ZIndex=5; FOVBox.Parent=MainFrame
...
local FOVTit=Instance.new("TextLabel"); FOVTit.Size=UDim2.new(0.5,0,0,18); FOVTit.Position=UDim2.new(0,9,0,3)
FOVTit.BackgroundTransparency=1; FOVTit.Text="FOV"; FOVTit.TextColor3=VIO
FOVTit.Font=Enum.Font.GothamBold; FOVTit.TextSize=11; FOVTit.TextXAlignment=Enum.TextXAlignment.Left
FOVTit.ZIndex=6; FOVTit.Parent=FOVBox
local FOVVal=Instance.new("TextLabel"); FOVVal.Size=UDim2.new(0.42,0,0,18); FOVVal.Position=UDim2.new(0.56,0,0,3)
FOVVal.BackgroundTransparency=1; FOVVal.Text="70"; FOVVal.TextColor3=VIO
FOVVal.Font=Enum.Font.GothamBold; FOVVal.TextSize=11; FOVVal.TextXAlignment=Enum.TextXAlignment.Right
...
local FovTrack=Instance.new("Frame"); FovTrack.Size=UDim2.new(1,-16,0,6); FovTrack.Position=UDim2.new(0,8,0,28)
...
local FovThumb=Instance.new("TextButton"); FovThumb.Size=UDim2.new(0,16,0,16); FovThumb.Position=UDim2.new(0.25,-8,0.5,-8)
...
SetFOV=function(fov)
    ...
    FovFill.Size=UDim2.new(pct,0,1,0); FovThumb.Position=UDim2.new(pct,-8,0.5,-8)
...
RunService.RenderStepped:Connect(function()
    if draggingFOV then
        ...
        SetFOV(FOV_MIN+pct*(FOV_MAX-FOV_MIN)) end
end)
SetFOV(70); Y=Y+50

-- REJOIN
local rejoinKey=nil; local rejoinListening=false
local RejoinBox=Instance.new("Frame"); RejoinBox.Size=UDim2.new(0,BW,0,50); RejoinBox.Position=UDim2.new(0.5,-(BW/2),0,Y)
RejoinBox.BackgroundColor3=BLACK; RejoinBox.BackgroundTransparency=0.55; RejoinBox.BorderSizePixel=0; RejoinBox.ZIndex=5; RejoinBox.Parent=MainFrame
...
local RejoinNameLbl=Instance.new("TextLabel"); RejoinNameLbl.Size=UDim2.new(1,0,0,22); RejoinNameLbl.Position=UDim2.new(0,9,0,0)
RejoinNameLbl.BackgroundTransparency=1; RejoinNameLbl.Text="REJOIN"; RejoinNameLbl.TextColor3=VIO
RejoinNameLbl.Font=Enum.Font.GothamBold; RejoinNameLbl.TextSize=11; RejoinNameLbl.TextXAlignment=Enum.TextXAlignment.Left
...
local RKeyBtn=Instance.new("TextButton"); RKeyBtn.Size=UDim2.new(1,-16,0,13); RKeyBtn.Position=UDim2.new(0,8,0,33)
...
RKeyBtn.Font=Enum.Font.Gotham; RKeyBtn.TextSize=7; RKeyBtn.BorderSizePixel=0; RKeyBtn.ZIndex=6; RKeyBtn.Parent=RejoinBox
...
RKeyBtn.MouseButton1Click:Connect(function()
    ...
    local conn; conn=UserInputSvc.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            ...
            RKeyBtn.Text="KEY: "..inp.KeyCode.Name.." (toca para cambiar)"; conn:Disconnect() end
    end)
end)
...
RejoinBox.InputBegan:Connect(function(inp)
    if inp.UserInputType==Enum.UserInputType.MouseButton1 then
        ...
        TweenService:Create(RejoinBox,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(40,0,70),BackgroundTransparency=0.1}):Play()
        RejoinStroke.Color=VIO_L; toggleStates.autorejoin=true; SaveSettings()
        pcall(function() TeleportSvc:Teleport(game.PlaceId,LocalPlayer) end)
    end
end)
Y=Y+58
...
local KitBox=Instance.new("Frame"); KitBox.Size=UDim2.new(0,BW,0,50); KitBox.Position=UDim2.new(0.5,-(BW/2),0,Y)
KitBox.BackgroundColor3=BLACK; KitBox.BackgroundTransparency=0.55; KitBox.BorderSizePixel=0; KitBox.ZIndex=5; KitBox.Parent=MainFrame
...
local KitNameLbl=Instance.new("TextLabel"); KitNameLbl.Size=UDim2.new(1,0,0,22); KitNameLbl.Position=UDim2.new(0,9,0,0)
KitNameLbl.BackgroundTransparency=1; KitNameLbl.Text="KIT"; KitNameLbl.TextColor3=VIO
KitNameLbl.Font=Enum.Font.GothamBold; KitNameLbl.TextSize=11; KitNameLbl.TextXAlignment=Enum.TextXAlignment.Left
...
local KKeyBtn=Instance.new("TextButton"); KKeyBtn.Size=UDim2.new(1,-16,0,13); KKeyBtn.Position=UDim2.new(0,8,0,33)
...
KKeyBtn.Font=Enum.Font.Gotham; KKeyBtn.TextSize=7; KKeyBtn.BorderSizePixel=0; KKeyBtn.ZIndex=6; KKeyBtn.Parent=KitBox
...
KKeyBtn.MouseButton1Click:Connect(function()
    ...
    local conn; conn=UserInputSvc.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            ...
            KKeyBtn.Text="KEY: "..inp.KeyCode.Name.." (toca para cambiar)"; conn:Disconnect() end
    end)
end)
...
KitBox.InputBegan:Connect(function(inp)
    if inp.UserInputType==Enum.UserInputType.MouseButton1 then
        ...
        if my>30 then return end
        toggleStates.autokit=true; SaveSettings(); DoKit()
    end
end)
Y=Y+58
    ...
    if s then
        ...
        if saved then
            ...
            if saved.brightness then
                ...
                toggleStates.brightness=true; ApplyToggleVisual("brightness",true) end
